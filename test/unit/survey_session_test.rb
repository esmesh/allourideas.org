...
class SurveySessionTest < ActiveSupport::TestCase
  ...
  context "the initialize method" do
    ...
    should "use given cookie_name" do
      s = SurveySession.new({:question_id => 1}, 'scispif_1_abc')
      assert_equal(s.cookie_name, 'scispif_1_abc')
    end
  ...
  context "the find method" do
    ...
    should "raise an exception when no cookie names match the question_id" do
      exception_test({:message => 'No possible cookies available', :args => [{'scispif_2_abc' => ''}, 1]})
    end

    should "raise an exception when the only possible cookie does not have a valid appearance lookup" do
      exception_test({:message => 'No cookie found valid with appearance_lookup', :args => [{'scispif_1_abc' => c({:question_id => 1})}, 1, 'lookup']})
    end

    should "raise an exception when the question id value does not match cookie name" do
      exception_test({:message => 'Question ID did not match cookie name', :args => [{'scispif_1_abc' => c({:question_id => 2})}, 1]})
    end

    should "raise an exception when the data is nil" do
      exception_test({:message => 'Data is not hash', :args => [{'scispif_1_abc' => c(nil)}, 1, 'lookup']})
    end

    should "raise an exception when the data is an empty string" do
      exception_test({:message => 'Data is not hash', :args => [{'scispif_1_abc' => c('')}, 1, 'lookup']})
    end

    should "raise an exception when cookie failed validation with no lookup" do
      exception_test({:message => 'All possible cookies failed verification', :args => [{"scispif_1_abc" => {}}, 1]})
    end

    should "raise an exception when cookie failed validation with a lookup" do
      exception_test({:message => 'No cookie found valid with appearance_lookup', :args => [{'scispif_1_abc' => {}}, 1, 'lookup']})
    end

    should "raise an exception when all possible cookies failed verification" do
      exception_test({:message => 'All possible cookies failed verification', :args => [{"scispif_1_abc" => {}, "scispif_1_def" => {}}, 1]})
    end

    should "raise an exception when multiple possible cookies found but are invalid" do
      exception_test({:message => 'No cookie found valid with appearance_lookup', :args => [{"scispif_1_abc" => {}, "scispif_1_def" => {}}, 1, 'lookup']})
    end

    should "raise an exception when the possible cookies don't have the proper lookup" do
      exception_test({:message => 'No cookie found valid with appearance_lookup', :args => [{"scispif_1_abc" => c({:question_id => 1}), "scispif_1_def" => {}}, 1, 'lookup']})
    end

    should "raise an exception when multiple possible cookies exist, but have nil data" do
      exception_test({:message => 'Data is not hash', :args => [{"scispif_1_abc" => c(nil), "scispif_1_def" => ''}, 1, 'lookup']})
    end

    should "raise an exception when the question id does not match cookie and a lookup is sent" do
      exception_test({:message => 'Question ID did not match cookie name', :args => [{'scispif_1_abc' => c({:question_id => 2, :appearance_lookup => 'lookup'})}, 1, 'lookup']})
    end
    ...
    should "return the session data when found" do
      ...
      test_matrix = [
        {:args => [{'scispif_1_abc' => c(match_session)}, 1], :result => [match_session, 'scispif_1_abc']},
        {:args => [{'scispif_1_abc' => c(match_session)}, '1'], :result => [match_session, 'scispif_1_abc']},
        {:args => [{'scispif_1_abc' => c(match_session), 'scispif_1_def' => c(no_match_session)}, 1], :result => [match_session, 'scispif_1_abc']},
        {:args => [{'scispif_1_abc' => c(match_session), 'scispif_2_aaa' => c(no_match_session)}, 1], :result => [match_session, 'scispif_1_abc']},
        {:args => [{'scispif_1_abc' => c(no_match_session), 'scispif_2_aaa' => c(match_2_session)}, 2], :result => [match_2_session, 'scispif_2_aaa']},
        {:args => [{'scispif_1_lookup' => c(match_session), 'scispif_1_aaa' => c(no_match_session)}, 1, 'lookup'], :result => [match_session, 'scispif_1_lookup']},
        {:args => [{'scispif_1_lookup' => c(match_session)}, 1, 'lookup'], :result => [match_session, 'scispif_1_lookup']},
        {:args => [{'scispif_1_lookup' => c(match_session)}, '1', 'lookup'], :result => [match_session, 'scispif_1_lookup']},
      ]
