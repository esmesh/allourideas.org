source "https://rubygems.org"
source "http://gems.github.com"

gem "pry"
gem 'pry-debugger', '~> 0.2.3'
gem 'roo', '~> 1.13.2'
gem 'spreadsheet', '0.9.8'
gem "rails", "2.3.18"
gem "rake", "~> 0.9.0"
gem "multi_json", "1.0.4"
gem "json"
gem 'libxml-ruby', :require => "libxml"
gem "ambethia-smtp-tls", "1.1.2", :require => "smtp-tls", :source => "https://rubygems.org"
gem "paperclip", "~> 2.7.0"
gem "mime-types", "1.16", :require => "mime/types"
gem "xml-simple", "1.0.12", :require => "xmlsimple"
gem "yfactorial-utility_scopes", "0.2.2", :require => "utility_scopes", :source => "https://rubygems.org"
gem "formtastic", "~> 0.2.2"
gem "inherited_resources",  "1.0.4"
gem "has_scope",  "0.4.2"
gem "responders",  "0.4.8"
gem "thoughtbot-clearance", "0.8.2", :require => "clearance", :source => "https://rubygems.org"
gem "fastercsv", "1.5.1", :platform => :ruby_18
gem "redis-store", "1.0.0.1"
gem "redis", "~> 2.2.1"
gem "system_timer", "~> 1.2.4", :platform => :ruby_18
gem "will_paginate", "2.3.14"
gem "geoip", "1.2.1"
gem "geokit", "1.5.0"
gem "crack", "0.1.4"
gem "friendly_id", "2.2.5"
gem "sax-machine", "0.0.14"
gem "i18n", "0.3.7"
gem "delayed_job", "2.0.6"
gem "haml", "~> 3.1.7"
gem "compass-rails", "~> 1.0.3"
gem "mysql2", "0.2.18"
gem "test-unit", "1.2.3"

