class Earl < ActiveRecord::Base
  @@reserved_names = ["questions", "question", 'about', 'privacy', 'tour', 'no_google_tracking', 'admin', 'earls', 'signup', 'sign_in', 'sign_out','clicks', 'fakequestion', 'photocracy', 'fotocracy']
  ...
  def self.voter_map(earl_name, type)
    if type == "all"
      votes_by_uids = Question.get(:all_num_votes_by_user_id, :scope => "all_votes")
    elsif type == 'all_photocracy_votes'
      votes_by_uids = Question.get(:all_num_votes_by_user_id, :scope => type)
    elsif type == 'all_aoi_votes'
      votes_by_uids = Question.get(:all_num_votes_by_user_id, :scope => type)
    elsif type == "all_creators"
      votes_by_uids = Question.get(:all_num_votes_by_user_id, :scope => "creators")
    elsif type == "uploaded_ideas"
         
      earl = Earl.find earl_name
      question = Question.new
      question.id = earl.question_id
      votes_by_uids = question.get(:object_info_by_user_id, :object_type => 'uploaded_ideas')
    elsif type == "bounces"
      earl = Earl.find earl_name
      question = Question.new
      question.id = earl.question_id
      votes_by_uids = question.get(:object_info_by_user_id, :object_type => 'bounces')

    elsif type == "votes"
      earl = Earl.find earl_name
      question = Question.new
      question.id = earl.question_id
      votes_by_uids = question.get(:object_info_by_user_id, :object_type => 'votes')
    end
     
    votes_by_geoloc= {}
    object_total = 0
    votes_by_uids.each do |vote|
      sid = vote["user_id"]
